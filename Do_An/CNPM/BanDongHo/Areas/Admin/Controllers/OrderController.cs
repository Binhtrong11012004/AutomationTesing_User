...
namespace BanDongHo.Areas.Admin.Controllers
{
    ...
    public class OrderController : Controller
    {
        ...
        {
            int startPageIndex = (pageIndex - 1) * 10;//mỗi trang 10 đơn hàng

            OrderViewModel orderViewModel = new OrderViewModel();
            orderViewModel.ListOrder = orderService.getAllOrder().Skip(startPageIndex).Take(10).ToList();//Skip bỏ qua đơn đầu lấy đơn hiện tại
        ...
        {         
            ...
            // load lại thông tin mà không cần load lại trang
            String html = "";
        ...
        public String UpdateOrder(OrderViewModel orderViewModel)
        {
            ...
            if(!orderViewModel.tinhtrang.Equals("đã hủy"))// kiểm tra trạng thái đơn hàng nếu đã hủy thì không kiểu tra số lượng tồn kho
            {
                for (int i = 0; i < count; i++)
                {
                    ...
                    int? soluongtonkho = orderService.TakeQuantityProduct(masp);// using? để chứ giá trị null 
            ...
            if (!kiemtra)//kiểm tra khác false thì countinue
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    totalMoney += soluong * sanpham.DONGIA;
                }
